...
class APIKeyStore(JSONStore):
    ...
    def sync(self, update_store=False, update_settings=False):
        ...
        _stored_values = (
            self.strip_details(*stored_values)
            if all(stored_values) else
            None
        )
        setting_values = (
            settings.api_key(),
        ...
        _setting_values = (
            self.strip_details(*setting_values)
            if all(setting_values) else
            None
        )
        ...
        if _stored_values:
            sync_to_settings = (
                    not update_store
                    and _setting_values != _stored_values
            )
        else:
        ...
        if _setting_values:
            sync_to_store = (
                    not update_settings
                    and _stored_values != _setting_values
            )
        else:
            sync_to_store = update_store

        update_store = _stored_values and stored_values != _stored_values
        update_settings = _setting_values and setting_values != _setting_values
        ...
        if sync_to_settings:
            ...
            settings.api_key(_setting_values[0])
            settings.api_id(_setting_values[1])
            settings.api_secret(_setting_values[2])

        elif sync_to_store:
            api_data['keys']['user'] = {
                'api_key': _setting_values[0],
                'client_id': _setting_values[1],
                'client_secret': _setting_values[2],
            }
            self.save(api_data, ipc=not forced)
